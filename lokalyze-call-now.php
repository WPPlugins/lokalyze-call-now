...
*/
/***************** Static *****************/
if ( !defined( 'LCN_PLUGIN_BASENAME' ) ) define( 'LCN_PLUGIN_BASENAME', plugin_basename( __FILE__ ) );
if ( !defined( 'LCN_PLUGIN_NAME' ) ) define( 'LCN_PLUGIN_NAME', trim( dirname( LCN_PLUGIN_BASENAME ), '/' ) );
if ( !defined( 'LCN_PLUGIN_URL' ) )	define( 'LCN_PLUGIN_URL', WP_PLUGIN_URL . '/' . LCN_PLUGIN_NAME );
/***************** Enqueuing Scripts and Styles For Frontend as well as backend *****************/
if(is_admin()){
	add_action('admin_enqueue_scripts', 'lcn_enqueue_style');
	if( ! function_exists( 'lcn_enqueue_style' ) ){
		function lcn_enqueue_style(){
			wp_register_style( 'lcn-admin-style', LCN_PLUGIN_URL.'/css/lcn_admin_style.css' );
			wp_enqueue_style( 'lcn-admin-style' );
			wp_enqueue_style( 'wp-color-picker' );
			wp_register_script( 'lcn-admin-js', LCN_PLUGIN_URL.'/js/lcn_adminjs.js', array( 'wp-color-picker' ), false, true );
			wp_localize_script( 'lcn-admin-js', 'ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ), 'we_value' => 1234 ) );
			wp_enqueue_script( 'lcn-admin-js' );
		}
	}
}else {
	add_action('wp_enqueue_scripts', 'lcn_enqueue_style');
	if( ! function_exists( 'lcn_enqueue_style' ) ){
		function lcn_enqueue_style(){
		}
	}
...
register_activation_hook( __FILE__, 'lokalyze_call_now_activate' );
function lokalyze_call_now_activate(){
	global $wp_version;
	...
	$table_name = $wpdb->prefix.'callnowdata';
...
register_uninstall_hook( __FILE__, 'lokalyze_call_now_uninstall' );
function lokalyze_call_now_uninstall(){
	global $wpdb;
	$wpdb->query($wpdb->prepare( "DROP TABLE IF EXISTS {$wpdb->prefix}callnowdata" ));
	$wpdb->query( "DROP TABLE IF EXISTS $table_name" );
}
/***************** Adding Admin Menus *****************/
add_action('admin_menu', 'lcb_setup_menu');
function lcb_setup_menu(){
...
/***************** Lokalyze Local SEO - CALL NOW BUTTON *****************/
...
function lcn_ajax_add_btnsettings(){
	...
	if( array_intersect($allowed_roles, $user->roles ) ){
		if( current_user_can('manage_options') ){
			...
			$callnowbutton = $butstatus."-|$|-".$butnumber."-|$|-".$butcolour."-|$|-".$butappear."-|$|-".$butcaltrk."-|$|-".$finaltrkcode;
			$wpdb->query($wpdb->prepare( "UPDATE wp_callnowdata SET callnowbutton = '$callnowbutton'" ));
		}
	}
...
function lcn_aloneanalyticshead(){
	global $wpdb;
	$callnowbuttons = $wpdb->get_results("SELECT * FROM wp_callnowdata");
			...
			function lcn_alonecallnowbutton_head(){
				...
				foreach($callnowbuttons as $callnowbut){
					...
					if($callnowbutton[3] == 'topr'){
						$ButtonAppearance = "width:60px;right:0;border-radius:5px;top:0px;";
						$ButtonExtra = "";
					}else if($callnowbutton[3] == 'topl'){
						$ButtonAppearance = "width:60px;left:0;border-radius:5px;top:0px;";
						$ButtonExtra = "";
					}else if($callnowbutton[3] == 'full'){
						$ButtonAppearance = "width:98%;left:0;bottom:0px;";
						$ButtonExtra = "body {padding-bottom:60px;}";
					}else if($callnowbutton[3] == 'left'){
						$ButtonAppearance = "width:60px;left:0;border-radius:5px;bottom:0px;";
						$ButtonExtra = "";
					}else {
						$ButtonAppearance = "width:60px;right:0;border-radius:5px;bottom:0px;";
						$ButtonExtra = "";
					}
					echo "<style>#lcn-callnowbutton {display:none;} @media screen and (max-width:980px){#lcn-callnowbutton {display:block; margin:1%; ".$ButtonAppearance." height:60px; position:fixed; border:2px solid ".lcn_alonechangecolor($callnowbutton[2],'lighter')."; background:url(data:image/svg+xml;base64,".lcn_alonesvg(lcn_alonechangecolor($callnowbutton[2], 'darker') ).") center 2px no-repeat ".$callnowbutton[2]."; text-decoration:none; box-shadow:0 0 5px #888; z-index:9999;background-size:58px 58px}".$ButtonExtra."}</style>";
				}
			}
				...
				foreach($callnowbuttons as $callnowbut){
					...
					if( in_array($currentday,$days_array) ){
						if( $hrformat == "12"){
							...
							$starttime = strtotime("$current_date_use $check_variable[0] hours $check_variable[1] minutes 0 seconds");
							$stdate = date('H:i:s', $starttime);
							...
							$endtime = strtotime("$current_date_use $check_variable[3] hours $check_variable[4] minutes 0 seconds");
							$enddate = date('H:i:s', $endtime);
							if(($currenttime >= $starttime) && ($currenttime <= $endtime)){
								echo '<a class="'.$currentday.'|'.$current_time.'|'.$stdate.'|'.$enddate.'-'.$currenttime.'|'.$starttime.'|'.$endtime.'" href="tel:'.$callnowbutton[1].'" id="lcn-callnowbutton" '.$tracking.'>&nbsp;</a>';
							}else {
								echo 'failed';
							}
						}else {
...
/***************** Lokalyze Local SEO Location Settings *****************/
	...
	if( array_intersect($allowed_roles, $user->roles ) ){
		if( current_user_can('manage_options') ){
			...
			if($open_hour_format == 12){
				$openinghoursdet = $wpdb->get_results($wpdb->prepare( "SELECT * FROM wp_callnowdata" ));
				foreach($openinghoursdet as $openhour){
					...
					if($openinghourscon != ""){
						...
						if($openhourformat == "12"){
							$wpdb->query($wpdb->prepare( "UPDATE wp_callnowdata SET openinghours = '$openinghours', openinghourformat = '$open_hour_format'" ));
						}else {
							...
							$wpdb->query($wpdb->prepare( "UPDATE wp_callnowdata SET openinghourformat = '$hrsformat', openinghours = '$openinghours'" ));
						}
					}else {
						$wpdb->query($wpdb->prepare( "UPDATE wp_callnowdata SET openinghours = '$openinghours', openinghourformat = '$open_hour_format'" ));
					}
...
function lcn_hours_function(){
	global $wpdb;
	$locationdetails = $wpdb->get_results("SELECT * FROM wp_callnowdata");
